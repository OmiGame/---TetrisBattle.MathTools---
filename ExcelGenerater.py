    ...
    def 生成所有关卡表格(cls, save_folder: str = save_folder_path_level, 表格样式: 表格样式类型 = 表格样式类型.默认样式) -> None:
        ...
        for 关卡编号, 关卡数据 in 关卡数据映射.items():
            ...
            try:
                ...
                # 检查是否是因为关卡数据不完整导致的失败
                if "关卡数据不完整" in 错误信息 or "数据缺失" in 错误信息:
                    print(f"关卡{关卡编号}因数据不完整跳过生成，耗时: {表格生成时间:.2f}秒")
                else:
                    # 记录非数据不完整的失败关卡
                    失败关卡列表.append({
                        '关卡编号': 关卡编号,
                    ...
                    print(f"关卡{关卡编号}生成失败，耗时: {表格生成时间:.2f}秒，错误: {错误信息}")
        ...
        print(f"\n{'='*60}")
        print(f"关卡表格生成完成统计:")
        print(f"总关卡数量: {len(关卡数据映射)}")
        print(f"成功生成数量: {成功生成数量}")
        print(f"失败数量: {len(失败关卡列表)}")
        print(f"总耗时: {总时间:.2f}秒")
        print(f"{'='*60}")
        ...
        # 打印失败关卡详情 - 放在最下方便于查看
        print(f"\n\n⚠️  生成失败的关卡详情 (共{len(失败关卡列表)}个):")
        print(f"{'='*80}")
        for i, 失败信息 in enumerate(失败关卡列表, 1):
            print(f"{i:2d}. 关卡编号: {失败信息['关卡编号']:>3} | 耗时: {失败信息['耗时']:>6.2f}秒")
            print(f"     失败原因: {失败信息['失败原因']}")
            if i < len(失败关卡列表):  # 不是最后一个时才打印分隔线
                print(f"     {'-'*70}")
        print(f"{'='*80}")
