...
class 阵容强度计算:
    ...
    @classmethod
    def 分配关卡阵容(cls, 所有阵容, 最大阵容强度百分比=0.56):
        """
        分配关卡阵容
        Args:
            所有阵容: 所有可能的阵容组合
            最大阵容强度百分比: 最大阵容强度百分比，默认为0.95，用来控制最高关卡的强度上限
        Returns:
            list: 关卡阵容列表
        """
        # 创建所有阵容的副本并按强度排序
        所有阵容副本 = sorted(所有阵容, key=lambda x: cls.计算阵容强度(x))
        
        # 获取强度范围
        最小强度 = cls.计算阵容强度(所有阵容副本[0])
        最大强度 = cls.计算阵容强度(所有阵容副本[-1]) * 最大阵容强度百分比
        强度范围 = 最大强度 - 最小强度
        最后一个阶段 = list(关卡阶段枚举)[-1]
        关卡数 = 最后一个阶段.value["结束关卡"]
        
        print(f"最小强度: {最小强度:.2f}, 最大强度: {最大强度:.2f}, 强度范围: {强度范围:.2f}, 关卡数: {关卡数}")
        
        # 计算每个关卡的目标强度（线性分布）
        目标强度列表 = [最小强度 + (强度范围 * i / (关卡数 - 1)) for i in range(关卡数)]
        
        # 打印部分目标强度以验证递增性
        for i in range(0, 关卡数, max(1, 关卡数 // 10)):
            print(f"关卡 {i+1} 目标强度: {目标强度列表[i]:.2f}")
        
        # 为每个关卡选择最接近目标强度的阵容
        关卡阵容 = []
        for 目标强度 in 目标强度列表:
            最接近的阵容 = min(所有阵容副本, key=lambda x: abs(cls.计算阵容强度(x) - 目标强度))
            关卡阵容.append(最接近的阵容)
        ...
        for i in range(关卡数):
            ...
            最终关卡强度.append(cls.计算阵容强度(最接近的阵容))
        
        # 验证最终强度是否递增
        递增 = True
        for i in range(1, 关卡数):
            if 最终关卡强度[i] < 最终关卡强度[i-1]:
                递增 = False
                print(f"警告: 关卡 {i+1} 的强度 {最终关卡强度[i]:.2f} 小于关卡 {i} 的强度 {最终关卡强度[i-1]:.2f}")
        
        if 递增:
            print("所有关卡强度均为递增")
        
        # 打印强度分布情况
        print(f"第1关强度: {最终关卡强度[0]:.2f}")
        print(f"第{关卡数//2}关强度: {最终关卡强度[关卡数//2-1]:.2f}")
        print(f"最终关强度: {最终关卡强度[-1]:.2f}")
        
        return 最终关卡阵容

    @classmethod
    def 获取所有关卡的强度列表(cls, 所有阵容) -> list[float]:
        """
        获取关卡强度列表
        ...
        # 检查强度列表长度
        最后一个阶段 = list(关卡阶段枚举)[-1]
        预期关卡数 = 最后一个阶段.value["结束关卡"]
        if len(关卡强度列表) != 预期关卡数:
            print(f"警告: 关卡强度列表长度 {len(关卡强度列表)} 与预期关卡数 {预期关卡数} 不一致")
        
        # 检查强度是否递增
        是否递增 = all(关卡强度列表[i] <= 关卡强度列表[i+1] for i in range(len(关卡强度列表)-1))
        print(f"关卡强度是否递增: {是否递增}")
    ...
    def 计算所有关卡角色强度要求(cls, 关卡强度列表: list[float]) -> dict:
        ...
        # 计算每个关卡的强度要求
        for 关卡数 in range(1, len(关卡强度列表) + 1):
            # 获取当前关卡所在阶段
            ...
            关卡角色强度要求[关卡数] = {}
            for 品质 in cls.角色品质:
                关卡角色强度要求[关卡数][品质] = 当前关卡强度 / 3 * 当前阶段.value["品质强度比例"][品质]
    ...
    def 计算阶段强度要求(cls, 阶段强度列表: dict) -> dict:
        ...
        for i, 关卡强度 in enumerate(强度列表):
            ...
            关卡角色强度要求[关卡数] = {}
            for 品质 in cls.角色品质:
                关卡角色强度要求[关卡数][品质] = 关卡强度 / 3 * 阶段.value["品质强度比例"][品质]
