...
class 资源累积图表生成器:
    ...
    def 处理需求数据为累积格式(self) -> Dict[str, List[float]]:
        ...
        for 日期 in 所有日期:
            ...
            # 提取当日各资源需求
            当日需求 = {
                "绿色碎片": 日需求.get("各品质碎片需求", {}).get("绿色", 0),
                "蓝色碎片": 日需求.get("各品质碎片需求", {}).get("蓝色", 0),  
                "紫色碎片": 日需求.get("各品质碎片需求", {}).get("紫色", 0),
                "金币": 日需求.get("每日金币总需求", 0)
            }
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 示例1：完全看广告情况，并且钻石都用来买精致宝箱的情况 (100%, 100%, 100%,0)
        生成玩家看广告情况图表(1, 1, 1, 0.2)
