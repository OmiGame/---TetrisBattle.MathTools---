...
波盈缺范围: Dict[波次体验, Tuple[float, float]] = {
    波次体验.轻松: (-10, -3),  # 轻松：负值较小，表示相对容易
    波次体验.一般: (-5, 5),   # 一般：接近平衡
    波次体验.困难: (2, 10),    # 困难：正值较小，表示稍微困难
    波次体验.地狱: (4, 11),    # 地狱：正值较大，表示非常困难
    波次体验.Boss: (-10, 5)     # Boss：正值中等，因为Boss不在战力计算范围中，所以其他怪物差不多就行，主要是纯战斗的boss的战力计算
}
